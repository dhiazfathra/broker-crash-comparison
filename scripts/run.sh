...
# The shared benchmark mutex is held by the CALLER for the whole matrix:
#   /tmp/expbrief/benchlock.sh acquire broker-crash-comparison
#   make bench
#   /tmp/expbrief/benchlock.sh release broker-crash-comparison
# This script must never acquire or release it: a script that releases a lock
# it does not own is exactly how three experiments once ran at the same time.
...
for b in $BROKERS; do
	for arm in $ARMS; do
		for run in $(seq 1 "$REPS"); do
			...
			h1=$(now_ns)
			c=$(docker exec "$CONSUMER_CT" /app/consumer -clock)
			h2=$(now_ns)
			offset=$((c - (h1 + h2) / 2))
			...
			if [ "$arm" = crash ]; then
				sleep "$KILL_AT"
				kill_ns=$(now_ns)
				docker kill -s SIGKILL "$CONSUMER_CT" >/dev/null
				sleep "$PAUSE"
				docker start "$CONSUMER_CT" >/dev/null
				restart_ns=$(now_ns)
			...
			docker cp "$CONSUMER_CT:/out/consumed.csv" results/current/consumed.csv >/dev/null
