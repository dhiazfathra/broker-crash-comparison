...
def steady(broker, _arm):
    """Median steady-state CPU% and RSS MiB of the broker container.

    Always taken from the CONTROL arm: the crash arm spends most of its wall
    clock idle, waiting out a redelivery timeout, so its mean CPU is an
    artefact of the outage rather than a cost of serving the load.
    """
    cpu, rss = [], []
    for p in glob.glob(f"results/runs/{broker}-control-run*.stats.csv"):
        ...
        for line in open(p):
            ...
            try:
                ...
                rss.append(
                    float(re.match(r"([\d.]+)([A-Za-z]+)", f[3]).group(1))
                    * {"KiB": 1 / 1024, "MiB": 1, "GiB": 1024}[
                        re.match(r"[\d.]+([A-Za-z]+)", f[3]).group(1)
                    ]
                )
            except Exception:
                pass
...
hdr = (
    "| broker | arm | n | published | delivered | lost | duplicates | dups after restart | "
    "thr msg/s | p50 ms | p95 ms | p99 ms | max ms | backlog peak | redeliv delay ms | "
    "catch-up ms | broker CPU % (control) | broker RSS MiB (control) |"
)
print(hdr)
print("|" + "---|" * (hdr.count("|") - 1))
for (b, arm), rs in sorted(runs.items()):
    ...
    print(
        f"| {b} | {arm} | {len(rs)} | {med(rs, 'published'):.0f} | {med(rs, 'deliveries'):.0f} | "
